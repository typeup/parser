...
function parse(source: Source): dom.Inline[] | undefined {
	...
	if (value == "\\") value = source.read()
	let region = source.mark()
	if (value && value != "\0") {
		result = inline.parse(source)
		if (!result) result = [new dom.Inline.Text(value, region) as dom.Inline]
		...
		} else result = [new dom.Inline.Text(value, region) as dom.Inline].concat(result)
	}
	return result
}
